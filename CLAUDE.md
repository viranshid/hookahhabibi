...
**Phase 1 complete (Authentication & Menu Browsing). Phase 3 in progress — Staff Menu shell with Tables / Menu / Orders tabs.**
...
## Packages
- `provider: ^6.1.2` — Managers provided via MultiProvider in main.dart. Use `context.watch<T>()` for reactive reads in build, `context.read<T>()` for mutations in event handlers.
- `cached_network_image: ^3.4.1` — Available for network images. Phase 1 uses custom `ImageCacheManager`.
